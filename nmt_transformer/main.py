...
wandb.init(
    project="Neural Machine Translation Transformer",
    entity="jenrola2292",
    name="first run",
)
wandb.config = {"learning_rate": 2e-5, "epochs": epochs, "batch_size": batch_size}


class EmbeddingEncoding(
    nn.Module
):  # the idea is to swap out this encoding layer with an lready pretrinad embedding model
...
class LearnedPositionalEncoding(
    nn.Module
):  # replacing fixed positional encoding with learnt positional encoding
...
class CrossAttention(nn.Module):
    ...
    def forward(
        self, input_ids, context
    ):  # input_ids from Decoder, context from Encoder
...
class FeedForward(nn.Module):
    def __init__(
        self, embedding_dim=embedding_dim, hidden_size=hidden_size, dropout_rate=0.1
    ):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, dropout_rate=0.1):
        ...
        self.enc_layers = nn.ModuleList([EncoderLayer() for i in range(num_layers)])
        self.dropout = nn.Dropout(dropout_rate)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.embedder = EmbeddingEncoding()
        self.pos_embedding = LearnedPositionalEncoding()
        self.dec_layers = nn.ModuleList([DecoderLayer() for i in range(num_layers)])
        self.ffn = FeedForward()
