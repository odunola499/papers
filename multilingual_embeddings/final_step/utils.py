...
def load_data_pairs():
    ...
    dataset = (
        dataset.map(lambda x: remove_nonetypes(x), batched=True)
        .shuffle()
        .select(range(150000))
    )
    dataset = dataset.map(
...
def load_data_embs():
    ...
    dataset = dataset.map(
        lambda x: tokenizer(
            x["premise"], max_length=256, padding="max_length", truncation=True
        ),
        batched=True,
    )
    ...
    dataset = dataset.map(
        lambda x: tokenizer(
            x["hypothesis"], max_length=256, padding="max_length", truncation=True
        ),
        batched=True,
    )
