...
class CompositeModel(nn.Module):
    def __init__(self):
        ...
        self.pool.load_state_dict(
            torch.load("multilingual_embeddings/final_layer.pth", map_location=device)
        )
...
for epoch in range(epochs):
    ...
    for batch in tqdm(loader, leave=True):
        ...
        student_mono_logits = student_model(mono_ids, mono_mask, mono_token_ids)[0][
            :, 0
        ]
        student_english_logits = student_model(
            english_ids, english_mask, english_token_ids
        )[0][:, 0]
        ...
        kld_mono_loss = kld_loss_func(
            F.log_softmax(student_mono_logits, dim=-1),
            F.softmax(reference_logits, dim=-1),
        )
        kld_english_loss = kld_loss_func(
            F.log_softmax(student_mono_logits, dim=-1),
            F.softmax(student_english_logits, dim=-1),
        )
